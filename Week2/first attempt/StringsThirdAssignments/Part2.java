...
public class Part2 {
    ...
    /**
     * Store all genes found in a dna strand and return a StorageResource with the
     * result.
     * 
    ...
    /**
     * Find index of start codon, starting search from a given position in dna
     * strand.
     * 
    ...
    /**
     * Helper function for checking if start and end indicies represents correct
     * gene sequence.
     * 
     * @param start index of start codon
     * @param end   index of end codon
     * @result true if number of elements between start and end indices are multiply
     *         of 3 false if end == - 1 or number of elements not a multiply of 3
     */
    ...
    /**
     * Find stop codon of correct gene, starting search from a given position in dna
     * strand.
     * 
     * @param dna   dna strand to be searched (should be in uppercase)
     * @param start start index from which dna string should be searched
     * @return index of a stop codon of a correct gene. Should return dna.lenght()
     *         if no correct gene is found in dna strand
     */
    public int findStopIndex(String dna, int start) {
        ...
        // for each codon type:
        // find stop codon and check if number of elements from start to codon index is
        // multiply of 3
        int tagIndex = dna.indexOf(END_TAG, start);
        int tagStopIndex = dna.length();
        if (isCorrectGene(start, tagIndex))
            tagStopIndex = tagIndex;

        int tgaIndex = dna.indexOf(END_TGA, start);
        int tgaStopIndex = dna.length();
        if (isCorrectGene(start, tgaIndex))
            tgaStopIndex = tgaIndex;

        int taaIndex = dna.indexOf(END_TAA, start);
        int taaStopIndex = dna.length();
        if (isCorrectGene(start, taaIndex))
            taaStopIndex = taaIndex;
    ...
    /**
     * Print the following information: prints all the Strings that are longer than
     * 60 characters prints the number of Strings that are longer than 60 characters
     * prints the Strings whose C-G-ratio is higher than 0.35 prints the number of
    ...
    public int getLongestGeneLength(StorageResource sr) {
        ...
        for (String gene : sr.data()) {
            if (gene.length() > maxLenght)
                maxLenght = gene.length();
        }
        return maxLenght;
    }
